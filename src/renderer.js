...
let workTime = 25 * 60 // 25分钟工作, 自行设定
let restTime = 5 * 60 // 5分钟休息
let state = {};
...
async function notification({title, body, actionText, closeButtonText, onclose, onaction}) {
    let res = await ipcRenderer.send('notification', {
        title,
        body,
...
function render () {
    ...
    switchButton.innerText = btnTextMap[type]
}
...
ipcRenderer.on('main-click-messages', (event, arg) => {
    arg === 1 ? startWork() : stopWork();
});
